...
zhang_fun <- function(X1, X2, B = 1000) {
  ...
  cor.dif1 <- (cor.hat1 * w1  - cor.hat2 * w2)
  cor.dif2 <- (transf1 - transf2)
  cor.dif3 <- (cor.hat1  - cor.hat2)

  stat_q_cor1_Fro <- sum(cor.dif1^2)
  stat_q_cor2_Fro <- sum(cor.dif2^2)
  stat_q_cor3_Fro <- sum(cor.dif3^2)
  
  stat_q_cor1_Ope <- max(abs(eigen(cor.dif1)$values))
  stat_q_cor2_Ope <- max(abs(eigen(cor.dif2)$values))
  stat_q_cor3_Ope <- max(abs(eigen(cor.dif3)$values))
  
  stat_q_cor1_Nuc <- sum(abs(eigen(cor.dif1)$values))
  stat_q_cor2_Nuc <- sum(abs(eigen(cor.dif2)$values))
  stat_q_cor3_Nuc <- sum(abs(eigen(cor.dif3)$values))
  
  stat_q_cor1_Max <- max(abs(cor.dif1))
  stat_q_cor2_Max <- max(abs(cor.dif2))
  stat_q_cor3_Max <- max(abs(cor.dif3))
  ...
  bt.stat_q_cor1_Fro <- rep(0, B)
  bt.stat_q_cor2_Fro <- rep(0, B)
  bt.stat_q_cor3_Fro <- rep(0, B)
  
  bt.stat_q_cor1_Ope <- rep(0, B)
  bt.stat_q_cor2_Ope <- rep(0, B)
  bt.stat_q_cor3_Ope <- rep(0, B)
  
  bt.stat_q_cor1_Nuc <- rep(0, B)
  bt.stat_q_cor2_Nuc <- rep(0, B)
  bt.stat_q_cor3_Nuc <- rep(0, B)
  
  bt.stat_q_cor1_Max <- rep(0, B)
  bt.stat_q_cor2_Max <- rep(0, B)
  bt.stat_q_cor3_Max <- rep(0, B)
  ...
  for (j in 1:B) {
    ...
    bt.cor.dif1   <- (bt.cor.hat1 * bt.w1 - bt.cor.hat2 * bt.w2)
    bt.cor.dif2   <- (bt.transf1 - bt.transf2)
    bt.cor.dif3   <- (bt.cor.hat1 - bt.cor.hat2)

    
    bt.stat_q_cor1_Fro[j] <- sum(bt.cor.dif1^2)
    bt.stat_q_cor2_Fro[j] <- sum(bt.cor.dif2^2)
    bt.stat_q_cor3_Fro[j] <- sum(bt.cor.dif3^2)
  
    bt.stat_q_cor1_Ope[j] <- max(abs(eigen(bt.cor.dif1)$values))
    bt.stat_q_cor2_Ope[j] <- max(abs(eigen(bt.cor.dif2)$values))
    bt.stat_q_cor3_Ope[j] <- max(abs(eigen(bt.cor.dif3)$values))
  
    bt.stat_q_cor1_Nuc[j] <- sum(abs(eigen(bt.cor.dif1)$values))
    bt.stat_q_cor2_Nuc[j] <- sum(abs(eigen(bt.cor.dif2)$values))
    bt.stat_q_cor3_Nuc[j] <- sum(abs(eigen(bt.cor.dif3)$values))
  
    bt.stat_q_cor1_Max[j] <- max(abs(bt.cor.dif1))
    bt.stat_q_cor2_Max[j] <- max(abs(bt.cor.dif2))
    bt.stat_q_cor3_Max[j] <- max(abs(bt.cor.dif3))
  ...
  zhang_q_cor1_pv_Fro <- mean(stat_q_cor1_Fro < bt.stat_q_cor1_Fro)
  zhang_q_cor2_pv_Fro <- mean(stat_q_cor2_Fro < bt.stat_q_cor2_Fro)
  zhang_q_cor3_pv_Fro <- mean(stat_q_cor3_Fro < bt.stat_q_cor3_Fro)
  
  zhang_q_cor1_pv_Ope <- mean(stat_q_cor1_Ope < bt.stat_q_cor1_Ope)
  zhang_q_cor2_pv_Ope <- mean(stat_q_cor2_Ope < bt.stat_q_cor2_Ope)
  zhang_q_cor3_pv_Ope <- mean(stat_q_cor3_Ope < bt.stat_q_cor3_Ope)
  
  zhang_q_cor1_pv_Nuc <- mean(stat_q_cor1_Nuc < bt.stat_q_cor1_Nuc)
  zhang_q_cor2_pv_Nuc <- mean(stat_q_cor2_Nuc < bt.stat_q_cor2_Nuc)
  zhang_q_cor3_pv_Nuc <- mean(stat_q_cor3_Nuc < bt.stat_q_cor3_Nuc)
  
  zhang_q_cor1_pv_Max <- mean(stat_q_cor1_Max < bt.stat_q_cor1_Max)
  zhang_q_cor2_pv_Max <- mean(stat_q_cor2_Max < bt.stat_q_cor2_Max)
  zhang_q_cor3_pv_Max <- mean(stat_q_cor3_Max < bt.stat_q_cor3_Max)
  

  mre <- c(
    zhang_q_cor1_pv_Fro, zhang_q_cor2_pv_Fro, zhang_q_cor3_pv_Fro,
    
    zhang_q_cor1_pv_Ope, zhang_q_cor2_pv_Ope, zhang_q_cor3_pv_Ope,
    
    zhang_q_cor1_pv_Nuc, zhang_q_cor2_pv_Nuc, zhang_q_cor3_pv_Nuc,
    
    zhang_q_cor1_pv_Max, zhang_q_cor2_pv_Max, zhang_q_cor3_pv_Max
  )
