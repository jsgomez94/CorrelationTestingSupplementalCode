...
.latex <- NULL
init <- TRUE
sp_options <- c("Sparse", "Dense")
measures_relevant <- c(12, 13, 10)

n_vals <- c(40, 80, 160, 320)
p_vals <- c(15, 30, 60, 120, 240, 480, 960)
p_vals_list <- list("15", "30", "60", "120", "240", "480", "960")

models_relevant <- c(1, 2, 3, 4, 5)
p_vals_relevant <- c(3, 4, 5, 6, 7)
n_vals_relevant <- c(1, 2, 3, 4)

p_rel_len <- length(p_vals_relevant)
n_rel_len <- length(n_vals_relevant)
...
coldef <- paste0(rep(" c ", p_rel_len), collapse ="")
pdef <- paste0(p_vals[p_vals_relevant], collapse = " & ")
.latex <- paste(
    .latex, "\\begin{center} \n \\resizebox{\\linewidth}{!}{% \n",
    "\\begin{tabular}{| c  c |", coldef, "|", coldef, "|", coldef, "| } \\hline \n")
.latex <- paste0(
    .latex, "\t\\multirow{2}{*}{Model} & \\multirow{2}{*}{Type} & ",
    "\\multicolumn{", p_rel_len, "}{c|}{ $\\|\\Rr_{1} - \\Rr_{2}\\|_F$ } & ",
    "\\multicolumn{", p_rel_len, "}{c|}{ $\\|\\Rr_{1} - \\Rr_{2}\\|_0$ } & ",
    "\\multicolumn{", p_rel_len, "}{c|}{ $\\|\\Rr_{1} - \\Rr_{2}\\|_{\\max}$ }  \\\\ \n")
.latex <- paste0(.latex, "\t & & ", pdef, " & ", pdef, " & ", pdef, "\\\\ \\hline\\hline \n")
cat(.latex)

for (.model in models_relevant) {
    .latex <- paste0(
        .latex, "\\multicolumn{2}{|c|}{} & ",
        "\\multicolumn{", p_rel_len, "}{c|}{ } & ",
        "\\multicolumn{", p_rel_len, "}{c|}{ } & ",
        "\\multicolumn{", p_rel_len, "}{c|}{ } \\\\ \n")
    .latex <- paste0(.latex, "\t\\multirow{2}{*}{Model ", .model,"}")
    
    for (sp_ind in 1:2){
        .latex <- paste0(.latex, " & ", sp_options[sp_ind])
        for (meas_ind in measures_relevant) {
            for (p_ind in p_vals_relevant) {
                rows <- which(p_col == p_vals[p_ind] & model_col == .model & sp_col == sp_ind)
                print(output_df[rows, meas_ind])
                val <- round(mean(output_df[rows, meas_ind]), digits = 2)
                .latex <- paste0(.latex, " & ", val)
            }
        }
        .latex <- paste0(.latex, "\\\\ \n")
    }
...
.latex <- paste0(
        ...
        "\\multicolumn{", p_rel_len, "}{c|}{ } & ",
        "\\multicolumn{", p_rel_len, "}{c|}{ } \\\\ \n")
.latex <- paste(.latex, "\\hline\\hline \n \\end{tabular}} \n \\end{center} \n \\end{table}\n\n\n")
.latex <- paste(.latex, "\\end{document}")
cat(.latex)

fileConn <- file(paste0(ROOT_PATH, subfolder_new, "results_mvn_cordiffs.tex"))
writeLines(.latex, fileConn)
close(fileConn)
rm(list = ls())
...
for (model_ind in 1:5) {
    file_name1 <- paste0(
        ...
        "PowerGapCov_model_m", model_ind, "_v", cov_counter,".pdf")
    ...
    file_name2 <- paste0(
        ROOT_PATH, subfolder_new,
        "PowerCov_model_m", model_ind, "_v", cov_counter,".pdf")
...
for (model_ind in 1:5) {
    file_name1 <- paste0(
        ROOT_PATH, subfolder_new,
        "PowerGapCorr_model_m", model_ind, "_v", cor_counter,".pdf")
