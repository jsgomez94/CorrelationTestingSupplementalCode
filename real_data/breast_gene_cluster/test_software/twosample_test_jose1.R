...
zhang_fun <- function(X1, X2, B, verbose = 100) {
  ...
  for (j in 1:B) {
    ...
    rm(
      b, b1, b2, Xb1, Xb2,
      bt.cov.hat1, bt.cov.hat2,
      bt.cor.hat1, bt.cor.hat2,
      bt.sd.vec1, bt.sd.vec2, bt.var.vec1, bt.var.vec2,
      bt.var.mat1, bt.var.mat2,
      bt.w1, bt.w2)
...
###### package inclusion found in "./requirements.R"
...
xeta_yeta_calculation_old <- function(x, y, verbose = FALSE) {
  if(verbose){
    print("Step 1: initial calculations")
  }
  n1 <- nrow(x)
  n2 <- nrow(y)
  p <- ncol(x)
  ...
  if(verbose){
    print("Step 2: calculating eta's")
  }
  
  for (k in 1:n1) {
    if (verbose) {print(paste("x,", k))}
    mat1x <- matrix(rep(xx[k, ]^2 / sig2x, p), ncol = p, byrow = TRUE)
    xa[, , k] <- ( (t(t(xx[k, ])) %*% t(xx[k, ])) / sqrt(t(t(sig2x)) %*% t(sig2x)) - ((xcor * mat1x)/ 2)  - ((xcor * t(mat1x)) / 2) )^2
  }
  
  for (k in 1: n2){
    if (verbose) {print(paste("y,", k))}
    mat1y <- matrix(rep(yy[k, ]^2 / sig2y, p), ncol = p, byrow = TRUE)
    ya[, , k] <- ( (t(t(yy[k, ])) %*% t(yy[k, ])) / sqrt(t(t(sig2y)) %*% t(sig2y)) - ((ycor * mat1y)/ 2)  - ((ycor * t(mat1y)) / 2) )^2
  }
  
  xeta = apply(xa, c(1, 2), mean)
  yeta = apply(ya, c(1, 2), mean)
  
  output <- list(xeta = xeta, yeta = yeta)
  return(output)
  
}

xeta_yeta_calculation_new <- function(x, y, verbose = FALSE) {
  if(verbose){
    print("Step 1: initial calculations")
  }
  n1 <- nrow(x)
...
cor_CaiZhang2016_fun <- function(x, y, eta_output, verbose = FALSE) {
  ...
  xeta <- eta_output$xeta
  yeta <- eta_output$yeta
  
  
  if(verbose) {
    print("Step 3: perform test")
  }
...
cor_ZhengEtAl2019_fun <- function(x, y, eta_output) { 
  ...
  pval_prime <- 1 - pnorm(stat_prime, mean = 0, sd = 1)
